...
\lhead{\small\color{monolight} SMART FLOW --- DETAILED TECHNICAL SPECIFICATION}
\rhead{\small\color{monolight} AUGUST 2026}
\lfoot{\small\color{monolight} Om Mukherjee, Abhishek Kotwani, Aryan Yadav}
...
\begin{center}
  ...
  {\large\itshape\color{monomedium}
    AI-Powered Adaptive Traffic Signal Control via Computer Vision \& Deep Reinforcement Learning
  }\\[9pt]
  \noindent\rule{\linewidth}{1.2pt}\\[6pt]
  {\small
    \textbf{Authors:} Om Mukherjee \quad Abhishek Kotwani \quad Aryan Yadav
    \quad\textbullet\quad
    \textbf{Event:} Smart India Hackathon 2024
  }\\[3pt]
  {\small\color{monolight}
    \url{https://github.com/meekhumor/AI-Based-Traffic-Signal-Control-System}
  }\\[8pt]
...
\subsection{Refactored Layout Mapping}

\begin{center}
\scriptsize
\begin{tabular}{@{}p{3.0cm} p{3.3cm}@{}}
\toprule
\textbf{Refactored path} & \textbf{Description} \\
\midrule
\texttt{cv/utils.py}
  & Shared contour detection \& cropping functions \\
\texttt{cv/lane\_detector.py}
  & Core Hough transform lane landmark API \\
...
\subsection{Annotated Project Directory Tree}
\begin{codebox}
\footnotesize
\begin{verbatim}
...
├── cv/
│   ├── __init__.py
│   ├── utils.py               # shared contours & crop
│   ├── lane_detector.py       # core lane detection API
│   ├── background_contour.py  # baseline noise estimation
│   ├── vehicle_counter.py     # contour counting script
│   ├── yolo_counter.py        # YOLOv8 alternative counter
│   ├── lane_debug_image.py    # debug visualizer (image)
│   ├── lane_debug_video.py    # debug visualizer (video)
│   ├── hough_explore_image.py # Hough slope exploration
│   └── hough_explore_video.py # Hough slope exploration
├── rl/
│   ├── __init__.py
│   ├── train_dqn.py           # DQN training script
│   ├── test_dqn.py            # SUMO-GUI evaluation script
│   └── saved_models/          # .zip model snapshots
├── sumo/
│   ├── kingcircle.sumocfg     # active SUMO config
│   └── maps/                  # .net.xml & .rou.xml files
├── notebooks/                 # Jupyter research notebooks
├── docs/                      # SIH2024 slides & PDF
├── yolov8n.pt                 # YOLOv8n pretrained weights
├── smart_flow.tex             # LaTeX technical specification
...
[Camera / Video File]
  |
  v
lane_detector.py  (Hough Lines)
  |--> top, left, right ROI points
  v
background_contour.py
  |--> back_contour baseline
  v
vehicle_counter.py
  |--> (avg - baseline) / 8 = vehicle_count
  v
[SUMO TraCI Env] <-- vehicle_count
  |
  _get_state()
  [halting, phase, time_remaining ...]
  |
  DQN Agent predict()
  |
  _take_action()
  [extend / switch / keep]
  |
  TraCI setPhaseDuration() / setPhase()
\end{verbatim}
\end{codebox}
...
% ==========================================
\section{Resolved Engineering Issues}
% ==========================================

\subsection{SyntaxError Fix in \texttt{rl/test\_dqn.py}}
Resolved the stray syntax error on line 105 (\texttt{obs, \_ = env.reset()q}
corrected to \texttt{obs, \_ = env.reset()}).

\subsection{Code Duplication Removal}
Extracted copy-pasted contour detection and image cropping functions into
a unified, reusable module \texttt{cv/utils.py}.

\subsection{Import Side-Effects Prevention}
Wrapped top-level execution code inside \texttt{if \_\_name\_\_ == '\_\_main\_\_':}
blocks in \texttt{cv/vehicle\_counter.py} and \texttt{cv/yolo\_counter.py}.

\subsection{Path Normalization}
Standardized relative config paths (\texttt{sumo/kingcircle.sumocfg}) and model
save/load locations (\texttt{rl/saved_models/}) across all modules.

% ==========================================
\section{Run Guide (Quick Reference)}
% ==========================================

\begin{enumerate}
  \item \textbf{Train DQN Model:}
\begin{codebox}
\footnotesize
\begin{verbatim}
...
\end{verbatim}
\end{codebox}
  \item \textbf{Evaluate Model in SUMO-GUI:}
\begin{codebox}
\footnotesize
\begin{verbatim}
...
\end{verbatim}
\end{codebox}
  \item \textbf{Run Contour Vehicle Counter:}
\begin{codebox}
\footnotesize
\begin{verbatim}
...
\end{verbatim}
\end{codebox}
  \item \textbf{Run YOLO Vehicle Counter:}
\begin{codebox}
\footnotesize
\begin{verbatim}
